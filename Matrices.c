...
entryNode *add_entry(entryNode *head, entryNode *newNode)
{
	if (head == NULL)
		head = newNode;
	else
...
coordinateNode *add_coordinate(coordinateNode *head, coordinateNode *newNode)
{
	if (head == NULL)
		head = newNode;
	else
...
{
	if (!correct_matrix_column_size(col_size, col_vectors) || !correct_row_size(row_size, col_vectors))
	{
		printf("Incorrect matrix dimension");
		return NULL;
	}
	Matrix *newMatrix = NULL;
	newMatrix = (Matrix *)calloc(1, sizeof(Matrix));

	if (newMatrix != NULL)
	{
		newMatrix->row_size = row_size;
		newMatrix->col_size = col_size;
		newMatrix->col_vectors = col_vectors;
	}
	return newMatrix;
}
